...
듀얼 모델은 단순히 모델을 두 개 실행하기 위한 구조가 아니라, 커스텀 휠체어 탐지 모델의 한계를 기본 YOLO 모델로 보완하기 위해 사용했습니다.

커스텀 모델은 프로젝트 목적에 맞게 `wheelchair`과 `wheelchair_user`를 집중적으로 탐지합니다. 하지만 실제 웹캠 환경에서는 의자, 벤치, 소파, 사람, 자전거처럼 휠체어와 모양이 비슷하거나 사람과 겹쳐 보이는 객체가 함께 등장합니다. 이때 커스텀 모델만 사용하면 일반 의자나 앉아 있는 사람을 `wheelchair_user`로 잘못 판단할 가능성이 있습니다.

그래서 `07_dual_model_webcam.py`에서는 두 모델을 함께 사용합니다.

- 커스텀 모델: `wheelchair`, `wheelchair_user` 탐지
- 기본 YOLOv8 모델: COCO 클래스 기반의 `person`, `chair`, `bench`, `couch` 등 일반 객체 탐지
- 두 모델의 박스가 겹치면 IoU와 overlap 기준으로 중복 표시를 줄임
- 의자류 객체와 겹치는 `wheelchair_user` 결과는 신뢰도가 충분히 높을 때만 유지해 오탐을 줄임

즉, 듀얼 모델 방식은 휠체어 관련 객체를 더 잘 잡기 위한 커스텀 모델과, 주변 일반 객체를 이해하는 기본 YOLO 모델을 함께 사용해서 실시간 웹캠 테스트의 안정성을 높이기 위한 설계입니다.
